...
export class Ai extends Character {
    ...
    constructor(name, color, difficulty, gameAssets) {
    ...
    makeInitialDecision(dist) {
        ...
        if (dist[0] === 'both') {
            let randomChoice = Math.floor(1 + Math.random() * 2);
            randomChoice === 1 ? this.move('down') : this.move('up');
            return randomChoice === 1 ? [true, 'y'] : [true, 'x'];
        };
    }
